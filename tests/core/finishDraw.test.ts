...
function makeGeoman(
  ...
  const toPair = (
    p: [number, number] | { lng: number; lat: number },
  ): [number, number] => (Array.isArray(p) ? p : [p.lng, p.lat]);
...
function makeEditor(
  mode: "polygon" | "massing" | "line" | null,
  geoman: unknown,
) {
...
describe("finishActiveLineOrPolygonDraw", () => {
  ...
  it("finishes massing through the polygon draw instance", () => {
    const { geoman, calls } = makeGeoman("polygon", [
      [0, 0],
      [1, 0],
      [1, 1],
    ]);
    const editor = makeEditor("massing", geoman);
